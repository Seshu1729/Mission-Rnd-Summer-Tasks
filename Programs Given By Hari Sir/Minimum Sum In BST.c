...
int get_minimum_sum(struct node *root)
{
	if (root == NULL)
		return 0;
	else if (root->left != NULL&&root->right != NULL)
		return root->data + min(get_minimum_sum(root->left), get_minimum_sum(root->right));
	else if (root->left!=NULL)
		return root->data + get_minimum_sum(root->left);
	else if (root->right != NULL)
		return root->data + get_minimum_sum(root->right);
	else
		return root->data;
}
...
int main()
{
	...
	printf("LEAST LEAF DISTANCE IS :: %d", get_minimum_sum(root));
	getchar();
	return 0;
}