...
def view_question(request,pk):
    ...
    #fetch user details
    user_details = User.objects.get(id=question_details.questioned_by_id)
    ...
    for answer in answers_list:
        ...
        #fetch comments
        comments = Comment.objects.filter(commented_to_id=answer["id"]).values()
        if len(comments)!=0:
...
def add_answer(request,pk):
    ...
    question_details.update(answer_count=F("answer_count") + 1)
...
def post_comment(request,pk):
    ...
    new_comment = Comment(comment_description=new_comment_description,commented_by=new_comment_by,commented_to=new_comment_to)
    new_comment.save()
...
def delete_question(request,pk):
    ...
    for answer in related_answers:
        Tag.objects.filter(commented_to_id=answer.id).delete()
    return HttpResponseRedirect("/home_page")
...
def delete_answer(request,pk):
    ...
    #delete comments
    Comment.objects.filter(commented_to_id=answer_details.id).delete()
